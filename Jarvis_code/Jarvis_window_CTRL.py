...
# App command map (Linux/Cross-platform friendly)
APP_MAPPINGS = {
    "notepad": "gedit" if os.name != 'nt' else "notepad",
    "calculator": "gnome-calculator" if os.name != 'nt' else "calc",
    "chrome": "google-chrome",
    "vlc": "vlc",
    "command prompt": "gnome-terminal" if os.name != 'nt' else "cmd",
    "control panel": "gnome-control-center" if os.name != 'nt' else "control",
    "settings": "gnome-control-center" if os.name != 'nt' else "start ms-settings:",
    "paint": "drawing" if os.name != 'nt' else "mspaint",
    "vs code": "code",
    "postman": "postman",
...
async def focus_window(title_keyword: str) -> bool:
    ...
    if os.name == 'nt':
        ...
        try:
            # Try to activate using wmctrl
            subprocess.run(["wmctrl", "-a", title_keyword], check=False)
            return True # Assume success if no error, as wmctrl doesn't return easy status
        except FileNotFoundError:
            logger.warning("wmctrl not installed. Cannot focus window on Linux.")
            return False
    ...
    if os.name == 'nt':
        ...
        try:
            subprocess.run(["wmctrl", "-c", window_title], check=False)
             ...
             return "❌ wmctrl not installed. Cannot close window on Linux."
